...
项目已公开发布到 [GitHub](https://github.com/hohohahayeahhei/ai-case-workbench)。2026-09-21 首次上传的 `d90cfba` 已通过 [GitHub Actions self-check](https://github.com/hohohahayeahhei/ai-case-workbench/actions/runs/35578363735)；工程自检通过不代表线上案例已经采集或核验成功。
...
当前浏览器任务需要实际在本地打开页面并记录观察；程序尚无无人值守浏览器驱动。2026-09-21 用户完成抖音登录后，已核对并导入 5 条真实作品的作者、页面发布时间和规范链接，其中 1 条超出最近一年范围。1 条 128.3 秒视频已完成本地中文转写，取得 67 段机器转写和 15 条 Codex 逐帧核对/作者声明证据；另外 3 条近期作品仍只有元数据。正式 API 首次转写失败后重试成功，尚未验收长期稳定性，机器转写中也已发现错识别。

真实证据已接入现有采集与四阶段审核。复审发现来源可信度、混用不同演示示例和夸大交互验证范围等问题，并触发重新抽取及核验；随后重评分遇到上游 HTTP 502，最终保留 `needs_scoring`，本轮抖音入选为 0。

本次改动与本地转写配置已部署到常驻服务。两次部署检查中精选数量均为 65；首次代码部署前后原有 411 条来源记录逐行不变，后续既有采集作业正常更新了部分历史来源记录。详见 [接入说明](docs/douyin-integration.md) 和 [验收记录](docs/douyin-validation-2026-09-21.md)。
